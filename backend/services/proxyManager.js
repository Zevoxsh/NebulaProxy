...
// Cross-worker broadcast: admin routes mutate a domain's config and call
// reloadProxy()/stopProxy() on whichever cluster worker happened to receive
// that HTTP request (cluster.SCHED_NONE). Every other worker keeps its own
// stale in-memory `proxies` entry — e.g. it can keep dialing a backend IP
// the user just changed away from, indefinitely, with no way to notice.
// Publishing the change here lets every worker converge on the same state.
const RELOAD_CHANNEL = 'nebula:proxymanager:change';
  ...
  async init(acmeManager = null) {
    ...
    if (config.cluster.enabled) {
      redisService.subscribe(RELOAD_CHANNEL, (message) => {
        let payload;
        try {
        ...
        const task = action === 'stop'
          ? this.stopProxy(domainId, { fromBroadcast: true })
          : this.reloadProxy(domainId, { fromBroadcast: true });

        task.catch((err) => {
          logger.error(`[ProxyManager] Cluster-broadcast ${action} failed for domain ${domainId}:`, err.message);
        });
      });
    }
  }
  ...
  /**
   * Notify sibling cluster workers that a domain's proxy config changed, so
   * they reload/stop it too instead of keeping stale in-memory state.
   */
  _broadcastChange(domainId, action) {
    if (!config.cluster.enabled) return;
    redisService.publish(RELOAD_CHANNEL, JSON.stringify({ domainId, action }));
  }
   ...
   */
  async stopProxy(domainId, { fromBroadcast = false } = {}) {
    ...
    if (!fromBroadcast) this._broadcastChange(id, 'stop');
   ...
   */
  async reloadProxy(domainId, { fromBroadcast = false } = {}) {
    ...
    try {
      ...
      // Stop if running (suppress its own broadcast — this reload already
      // broadcasts once for the whole operation below)
      if (oldEntry) {
        await this.stopProxy(numId, { fromBroadcast: true });
      }
      ...
      if (!fromBroadcast) this._broadcastChange(numId, 'reload');
    } finally {
      this.reloadLock = false;
    }
  }
  ...
  async stopAll() {
    ...
    for (const id of ids) {
      promises.push(this.stopProxy(id, { fromBroadcast: true }));
    }
