    ...
    try {
      switch (proxyType) {
        ...
        case 'http':
          ...
          break;
        case 'minecraft':
          if (domain.minecraft_edition === 'bedrock') {
            await this._startUdpProxy(domain);
          } else {
  ...
  _startUdpProxy(domain) {
    ...
    serverSocket.on('message', async (msg, rinfo) => {
      ...
      if (!upstreamEntry) {
        ...
        // DDoS protection (new connections only)
        if (!upstreamEntry && domain?.ddos_protection_enabled) {
          try {
            const { ddosProtectionService } = await import('./ddosProtectionService.js');
            const ddosResult = await ddosProtectionService.check(rinfo.address, domain.id, domain);
            if (ddosResult.blocked) return; // silently drop
          } catch (_) {}
        }
...
<html lang="en">
  ...
  <body>
    <div class="card">
      ...
      <h1>Upstream service unavailable</h1>
      <p>The proxy cannot reach the backend for this domain right now. This usually happens when the service is offline, the upstream port is closed, or the backend is temporarily restarting.</p>
      <div class="grid">
        <div class="tile">
          <span>Domain</span>
      ...
      <footer>Contact your administrator if this persists. Timestamp: ${new Date().toISOString()}</footer>
    </div>
  </body>
</html>`;
  }
