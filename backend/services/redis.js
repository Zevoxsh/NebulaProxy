...
class RedisService {
  ...
  /**
   * Publish a message to a channel — best-effort, used to broadcast
   * state changes (e.g. proxy config reloads) to every cluster worker,
   * since each worker is a separate process with its own memory.
  ...
  async publish(channel, message) {
    ...
    try {
      ...
      logger.error(`[Redis] Publish to ${channel} failed:`, err.message);
    }
  }

  /**
   * Subscribe to a channel. Uses a dedicated duplicated connection because
   * an ioredis client that issues SUBSCRIBE can no longer run other
   * commands (this.client stays free for everything else).
   * @param {string} channel
  ...
  subscribe(channel, handler) {
    ...
    subscriber.on('error', (err) => {
      logger.error(`[Redis] Subscriber error (${channel}):`, err.message);
    });
    subscriber.subscribe(channel).catch((err) => {
      logger.error(`[Redis] Failed to subscribe to ${channel}:`, err.message);
    });
