...
<div id="nb-cel" role="status" aria-live="polite" aria-hidden="true">
  <div class="nb-cel-card">
    <div class="nb-cel-figure">
      <img class="nb-cel-mascot" src="` + MASCOT_DATA_URI + `" alt="" aria-hidden="true">
...
.nb-cel-figure{position:relative;width:96px;height:96px;margin-bottom:14px}
.nb-cel-mascot{width:96px;height:96px;display:block;filter:invert(1)}
@media (prefers-color-scheme:dark){.nb-cel-mascot{filter:none}}
#nb-cel.show .nb-cel-mascot{animation:nb-pop .5s cubic-bezier(.2,1.2,.4,1) both}
.nb-cel-check{position:absolute;right:-8px;bottom:-4px;width:38px;height:38px;background:#fff;border-radius:50%}
