...
export class MultiProxySyncService {
  ...
  init(proxyManagerInstance, databaseInstance) {
    ...
    logger.info(`[MultiProxySync] Initialized with process ID: ${this.processId}`);
    return this;
  }
  ...
  async startListening() {
    ...
    this.notificationHandler = async (message) => {
      ...
      try {
        ...
        logger.error(`[MultiProxySync] Failed to handle sync event:`, err.message);
      }
    };

    this.pgClient.on('notification', this.notificationHandler);
    this.pgClient.on('error', (err) => {
      logger.error('[MultiProxySync] PostgreSQL listener error:', err.message);
      this._scheduleReconnect();
    });
    this.pgClient.on('end', () => {
      if (!this._stopping) {
        logger.warn('[MultiProxySync] PostgreSQL listener connection ended unexpectedly, reconnecting...');
        this._scheduleReconnect();
      }
    });
    ...
    logger.info(`[MultiProxySync] Now listening for domain changes on PostgreSQL channel: ${this.subscriptionChannel}`);
  }
  ...
  async stopListening() {
    ...
    if (this.pgClient) {
      try {
        ...
        logger.warn(`[MultiProxySync] Failed to unlisten:`, err.message);
      }
    ...
    this.isSubscribed = false;
    logger.info(`[MultiProxySync] Stopped listening`);
  }
  ...
  _scheduleReconnect(delayMs = 5000) {
    ...
    this._reconnectTimer = setTimeout(async () => {
      ...
      try {
        ...
        logger.info('[MultiProxySync] Reconnected to PostgreSQL listener successfully');
      } catch (err) {
        logger.error('[MultiProxySync] Reconnect failed:', err.message);
  ...
  async publishDomainChange(action, domain, details = {}) {
    ...
    try {
      ...
      logger.info(`[MultiProxySync] Published '${action}' event for domain ${domain.hostname}`);
    } catch (err) {
      logger.error(`[MultiProxySync] Failed to publish event:`, err.message);
    }
  }
  ...
  async _handleSyncEvent(event) {
    ...
    logger.info(`[MultiProxySync] Received event from proxy ${event.sourceProxyId}: ${action} for ${domain.hostname}`);
    ...
    try {
      switch (action) {
        ...
        default:
          logger.warn(`[MultiProxySync] Unknown action: ${action}`);
      }
    } catch (err) {
      logger.error(`[MultiProxySync] Error handling ${action} event:`, err.message);
    }
  }
  ...
  async _syncDomainCreated(domainInfo) {
    try {
      ...
      if (!domain) {
        logger.warn(`[MultiProxySync] Domain not found in database: ${domainInfo.id}`);
        return;
      }
      ...
      if (existing) {
        logger.info(`[MultiProxySync] Domain ${domain.hostname} already running locally`);
        return;
      }

      // If domain is active, start it
      if (domain.is_active) {
        logger.info(`[MultiProxySync] Starting domain ${domain.hostname} (created on another proxy)`);
        await this.proxyManager.startProxy(domain);
      }
    } catch (err) {
      logger.error(`[MultiProxySync] Failed to sync domain creation:`, err.message);
    }
  }
  ...
  async _syncDomainModified(domainInfo) {
    try {
      ...
      if (!existing && domain.is_active) {
        logger.info(`[MultiProxySync] Starting previously inactive domain ${domain.hostname}`);
        await this.proxyManager.startProxy(domain);
      }
      // If it was active and is now inactive, stop it
      else if (existing && !domain.is_active) {
        logger.info(`[MultiProxySync] Stopping domain ${domain.hostname} (now inactive)`);
        await this.proxyManager.stopProxy(domain.id);
      }
      // If it's still active, reload it (in case settings changed)
      else if (existing && domain.is_active) {
        logger.info(`[MultiProxySync] Reloading domain ${domain.hostname}`);
        await this.proxyManager.stopProxy(domain.id);
        await this.proxyManager.startProxy(domain);
      }
    } catch (err) {
      logger.error(`[MultiProxySync] Failed to sync domain modification:`, err.message);
    }
  }
  ...
  async _syncDomainDeleted(domainInfo) {
    try {
      ...
      if (existing) {
        logger.info(`[MultiProxySync] Stopping deleted domain ${domainInfo.hostname}`);
        await this.proxyManager.stopProxy(domainInfo.id);
      }
    } catch (err) {
      logger.error(`[MultiProxySync] Failed to sync domain deletion:`, err.message);
    }
  }
  ...
  async _syncFullReload() {
    try {
      logger.info(`[MultiProxySync] Starting full reload of all domains`);
      ...
      for (const [domainId, entry] of this.proxyManager.proxies) {
        if (!activeDomainsInDb.has(domainId)) {
          ...
          try {
            ...
            logger.warn(`[MultiProxySync] Failed to stop proxy ${domainId}:`, err.message);
          }
        }
      }
      ...
      for (const domain of allDomains) {
        if (domain.is_active) {
          ...
          if (!isRunning) {
            logger.info(`[MultiProxySync] Starting domain ${domain.hostname} (not running locally)`);
            try {
              await this.proxyManager.startProxy(domain);
            } catch (err) {
              logger.warn(`[MultiProxySync] Failed to start proxy for ${domain.hostname}:`, err.message);
            }
          }
        }
      }

      logger.info(`[MultiProxySync] Full reload completed`);
    } catch (err) {
      logger.error(`[MultiProxySync] Failed to perform full reload:`, err.message);
    }
  }
  ...
  async forceFullReload() {
    logger.info(`[MultiProxySync] Force reload triggered`);
    await this._syncFullReload();
  }
