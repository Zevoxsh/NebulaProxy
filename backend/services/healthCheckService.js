 ...
 * `hostHeader`, when given, is sent as the Host header instead of the raw
 * backend host/IP — apps that validate Host (Laravel's ALLOWED_HOSTS-style
 * check, etc.) 400 on the bare IP, which reads as a failed probe even though
 * the backend is fine.
 * @returns {{ success: boolean, responseTime: number, statusCode: number|null, error: string|null }}
 */
function checkHttp(host, port, proto, timeoutMs, hostHeader) {
...
class HealthCheckService {
  ...
  async runOnce() {
    ...
    let lbBackends;
    try {
      lbBackends = await database.getAllActiveBackendsForHealthCheck();
    } catch (err) {
      logger.error('[HealthCheck] Failed to fetch load-balanced backends:', err.message);
      lbBackends = [];
    }

    if (!domains?.length && !lbBackends?.length) return;

    const concurrency  = config.healthChecks.concurrency || 10;
    const timeoutMs    = config.healthChecks.timeoutMs    || 10_000;
    const t0           = Date.now();

    // Run both queues (domains + per-backend LB entries) through the same
    // bounded-concurrency semaphore so the two probes never combine to more
    // than `concurrency` in-flight sockets at once.
    const jobs = [
      ...(domains || []).map((domain) => () => this._checkDomain(domain, timeoutMs)),
      ...(lbBackends || []).map((backend) => () => this._checkBackend(backend, timeoutMs)),
    ];
  ...
  async _checkDomain(domain, timeoutMs) {
    ...
    try {
      if (proxyType === 'udp') {
        ...
        result = await checkHttp(host, port, proto, timeoutMs, domain.hostname);
        ...
        if (!result.success && !hasExplicitScheme) {
          ...
          const altResult = await checkHttp(host, port, altProto, timeoutMs, domain.hostname);
  ...
  /**
   * Probe a single load-balancing backend row (domain_backends) and record
   * its status into backend_health_status — this is what feeds the
   * per-backend badge in the Load Balancing panel.
   */
  async _checkBackend(backend, timeoutMs) {
    const proxyType = (backend.proxy_type || 'http').toLowerCase();

    if (proxyType === 'udp' && config.healthChecks.skipUdp) return;

    const backendUrl  = backend.backend_url  || '';
    const backendPort = backend.backend_port || null;
    ...
    try {
      if (proxyType === 'udp') {
        const { host, port } = parseBackend(backendUrl, backendPort, 'udp', 0);
        if (!port) throw new Error('No UDP port configured for this backend');
        result = await checkUdp(host, port, timeoutMs);
      } else if (proxyType === 'tcp' || proxyType === 'minecraft') {
        const defaultPort = proxyType === 'minecraft' ? 25565 : 443;
        const { host, port } = parseBackend(backendUrl, backendPort, 'tcp', defaultPort);
        ...
        const hasExplicitScheme = /^https?:\/\//i.test(backendUrl);
        const defaultProto = backend.ssl_enabled ? 'https' : 'http';
        const { host, port, proto } = parseBackend(backendUrl, backendPort, defaultProto, 80);
        result = await checkHttp(host, port, proto, timeoutMs, backend.domain_hostname);

        if (!result.success && !hasExplicitScheme) {
          const altProto = proto === 'https' ? 'http' : 'https';
          const altResult = await checkHttp(host, port, altProto, timeoutMs, backend.domain_hostname);
          if (altResult.success) result = altResult;
        }
      }
    } catch (err) {
    ...
    try {
      await database.updateBackendHealthStatus(backend.id, status, result.responseTime);
    } catch (err) {
      logger.error({ error: err }, `[HealthCheck] DB write failed for backend ${backend.id}:`);
    }
  }
