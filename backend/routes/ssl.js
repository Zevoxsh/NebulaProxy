...
export async function sslRoutes(fastify, _options) {
  ...
  // POST /ssl/wildcards/request-acme - Initiate Let's Encrypt DNS-01 for a wildcard cert
  fastify.post('/wildcards/request-acme', {
    onRequest: [fastify.authenticate],
    handler: async (request, reply) => {
      try {
        if (request.user.role !== 'admin') {
        ...
        const email = acmeManager.getEmail();
        if (!email) {
          return reply.code(400).send({ error: 'ACME email is not configured. Set it in Settings → SSL.' });
        }

        // Find or create the wildcard cert record (placeholder, cert data filled after validation)
        let cert = await database.getWildcardCertByHostname(hostname);
        if (!cert) {
          cert = await database.storeWildcardCert({
            hostname,
            fullchain: null,
            privateKey: null,
            issuer: "Let's Encrypt (pending)",
        ...
        // If a challenge is already pending, return it
        if (cert.dns_challenge_status === 'waiting_user' &&
            cert.dns_challenge_expires_at &&
            new Date(cert.dns_challenge_expires_at) > new Date()) {
          return reply.send({
            success: true,
            alreadyPending: true,
            wildcardCertId: cert.id,
            challenge: {
              txtRecord: cert.dns_challenge_domain,
              txtValue: cert.dns_challenge_token,
              expiresAt: cert.dns_challenge_expires_at
            }
          });
        }

        const { token, domain: txtDomain } = await acmeManager.initiateWildcardACME(hostname, cert.id);

        fastify.log.info({ hostname, certId: cert.id }, 'Wildcard ACME challenge initiated');

        return reply.send({
          success: true,
          wildcardCertId: cert.id,
          message: 'DNS-01 challenge initiated. Create the TXT record then click Validate.',
          challenge: {
            txtRecord: txtDomain,
            txtValue: token,
            hostname,
        ...
        fastify.log.error({ error }, 'Failed to initiate wildcard ACME challenge');
        return reply.code(500).send({ error: error.message || 'Internal Server Error' });
      }
    }
  });

  // POST /ssl/wildcards/:id/validate-acme - Complete wildcard DNS-01 validation
  fastify.post('/wildcards/:id/validate-acme', {
    onRequest: [fastify.authenticate],
    handler: async (request, reply) => {
      try {
        if (request.user.role !== 'admin') {
        ...
        if (!cert.dns_challenge_status || cert.dns_challenge_status === 'failed') {
          return reply.code(400).send({ error: 'No pending DNS challenge for this wildcard cert. Initiate a new request first.' });
        }

        const { expiresAt } = await acmeManager.validateWildcardACME(cert.id, cert.hostname);

        certificateManager.invalidateWildcardCacheEntries(cert.hostname);
        fastify.log.info({ hostname: cert.hostname }, 'Wildcard ACME certificate issued successfully');

        return reply.send({
          success: true,
          message: `Wildcard certificate for ${cert.hostname} issued successfully`,
          expiresAt
        });
      } catch (error) {
        fastify.log.error({ error }, 'Failed to validate wildcard ACME challenge');
        return reply.code(500).send({ error: error.message || 'Internal Server Error' });
      }
    }
  });

  // POST /ssl/wildcards/:id/cancel-acme - Cancel a pending wildcard DNS-01 challenge
  fastify.post('/wildcards/:id/cancel-acme', {
    onRequest: [fastify.authenticate],
    handler: async (request, reply) => {
      try {
        if (request.user.role !== 'admin') {
        ...
        await acmeManager.cancelWildcardACME(cert.id);
        return reply.send({ success: true });
      } catch (error) {
        fastify.log.error({ error }, 'Failed to cancel wildcard ACME challenge');
        return reply.code(500).send({ error: error.message || 'Internal Server Error' });
      }
    }
  });

  // GET /ssl/wildcards/:id/check-dns - Check if TXT record has propagated
  fastify.get('/wildcards/:id/check-dns', {
    onRequest: [fastify.authenticate],
    handler: async (request, reply) => {
      try {
        if (request.user.role !== 'admin') {
        ...
        const { id } = request.params;
        const cert = await database.getWildcardCertById(id);
        if (!cert || !cert.dns_challenge_domain || !cert.dns_challenge_token) {
          return reply.code(400).send({ error: 'No pending DNS challenge for this wildcard cert' });
        }

        const propagated = await acmeManager.checkDNSPropagation(
          cert.dns_challenge_domain,
          cert.dns_challenge_token
        );

        return reply.send({ propagated, txtRecord: cert.dns_challenge_domain, txtValue: cert.dns_challenge_token });
      } catch (error) {
        fastify.log.error({ error }, 'Failed to check DNS propagation');
        return reply.code(500).send({ error: error.message || 'Internal Server Error' });
      }
    }
  });
