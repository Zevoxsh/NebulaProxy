...
import { getWebauthnContext } from './auth/helpers.js';
...
const pendingPasskeyRegistrations = createPendingChallengeStore('register', 10 * 60);
...
export async function userRoutes(fastify, _options) {
  ...
  fastify.post('/passkeys/register/options', {
    ...
    await pendingPasskeyRegistrations.set(request.user.id, {
  ...
  fastify.post('/passkeys/register/verify', {
    ...
    const pending = await pendingPasskeyRegistrations.get(request.user.id);
    if (!pending) {
      return reply.code(400).send({
        success: false,
    ...
    try {
      ...
      await pendingPasskeyRegistrations.delete(request.user.id);
      reply.send({
        success: true,
      ...
      fastify.log.error(
        { error, expectedRPID: pending.rpID, expectedOrigin: pending.origin },
