...
// of trustProxy (config.security.trustedProxies) — reusing them instead of
// re-parsing x-forwarded-host/x-forwarded-proto by hand keeps this in sync
// with the rest of the app's proxy-trust behavior instead of drifting from it.
export function getWebauthnContext(request) {
  ...
  const origin = `${request.protocol}://${cleanHost}`;
  return { rpID, origin };
}
