...
const pendingPasskeyAuthentications = createPendingChallengeStore('auth', 10 * 60);
...
export async function passkeyRoutes(fastify, _options) {
  ...
  fastify.post('/passkey/options', {
    ...
    await pendingPasskeyAuthentications.set(options.challenge, {
      origin,
      rpID
    });
  ...
  fastify.post('/passkey/verify', {
    ...
    const pending = await pendingPasskeyAuthentications.get(decodedClientData.challenge);
    if (!pending) {
      return reply.code(400).send({
        success: false,
    ...
    try {
      ...
      await pendingPasskeyAuthentications.delete(decodedClientData.challenge);
      await pool.query(
